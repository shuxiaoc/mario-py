## read in 
path0 = "../Benchmarking/data/delete_cell_types/sxc_bm/"
path = "../Benchmarking/data/delete_cell_types/"
x = read.csv(paste0(path0,"orig_x.csv"))
y = read.csv(paste0(path0,"orig_y.csv"))
## some parameters
celltype_2_delete = c('pDCs','NK','B','CD8 T','CD4 T','monocyte')
path_out = paste0(path,"seurat_bm/")
feature_2_delete = c("Full",celltype_2_delete)
## container
coluse = c("n_deleted","1v1_acc","transfer_acc","prop_remain","celltype_error")
...
for (item in feature_2_delete){
  ...
  if (item != "Full"){ # drop the current feature
    # drop rows of that cell type
    #x_use = x[x_label != item,]
    y_use = y[y_label != item,]
    #x_label_use = x_label[x_label!=item]
    y_label_use = y_label[y_label!=item]
  }
  ...
  ## test run pca
  y_obj = RunPCA(y_obj, features = features, npcs = (length(features)-1))
  x_obj = RunPCA(x_obj, features = features, npcs = (length(features)-1))
  ...
  predictions <- TransferData(anchorset = pre.anchors, refdata = y_label_use, 
  ...
  matchmm$dXlabel=x_label_use[matchmm$dX]
  matchmm$dYlabel=y_label_use[matchmm$dY]
  # remain porp
  rem_prop = length(unique(matchmm$dX)) / dim(x)[1] # 50%
  container[idx,"prop_remain"] = rem_prop
  # 1v1 match
  ovoM = sum(matchmm$dXlabel==matchmm$dYlabel)/dim(matchmm)[1] # 0.9
  container[idx,"1v1_acc"] = ovoM
  # knn label trans
  knnM = sum(predictions$predicted.id==x_label_use)/dim(x)[1] # 0.94
  container[idx,"transfer_acc"] = knnM
  # drop cell type number
  dropcell = length(x_label_use[x_label_use == item])
  dropcell_match = length(matchmm$dXlabel[matchmm$dXlabel == item])
  error_match = dropcell_match/dropcell
  container[idx,"celltype_error"] = error_match
  # embedding
...
write.csv(container,"../Benchmarking/data/delete_cell_types/seurat_bm/metrics.csv")
...
## some parameters
celltype_2_delete = c('pDCs','NK','B','CD8 T','CD4 T','monocyte')
path_out = paste0(path,"fstmnn_bm/")
feature_2_delete = c("Full",celltype_2_delete)
## container
coluse = c("n_deleted","1v1_acc","prop_remain","celltype_error")
container = matrix(NA, nrow = length(feature_2_delete), ncol = length(coluse))
container = as.data.frame(container)
colnames(container) = coluse
## label
...
for (item in feature_2_delete){
  ...
  mnnMatch$dXlabel=x_label_use[mnnMatch$dx]
  mnnMatch$dYlabel=y_label_use[mnnMatch$dy]
  
  # remain porp
  rem_prop = length(unique(mnnMatch$dx)) / dim(x)[1]
  container[idx,"prop_remain"] = rem_prop
  # 1v1 match
  ovoM = sum(mnnMatch$dXlabel==mnnMatch$dYlabel)/dim(mnnMatch)[1]
  container[idx,"1v1_acc"] = ovoM
  
  # drop cell type number
  dropcell = length(x_label_use[x_label_use == item])
  dropcell_match = length(mnnMatch$dXlabel[mnnMatch$dXlabel == item])
  error_match = dropcell_match/dropcell
  container[idx,"celltype_error"] = error_match
  # embedding
  embedding = xy_int@reductions$pca@cell.embeddings # pca scores
...
write.csv(container,"../Benchmarking/data/delete_cell_types/fstmnn_bm/metrics.csv")
