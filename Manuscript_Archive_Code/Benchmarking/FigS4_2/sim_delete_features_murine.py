...
if __name__ == "__main__":
    ...
    path = 'data/feature_deletion/murine/sxc_5k_20k'
    os.makedirs(path, exist_ok=True)

    np.random.seed(42)
    df1 = pd.read_csv('data/murineCodex_forSIM.csv')
    df2 = pd.read_csv('data/murineCite_forSIM.csv')
    n1, _ = df1.shape
    n2, _ = df2.shape
    ...
    run_delete_features(df1, df2, x_labels, y_labels, features_to_delete,
                        ...
                        top_k=3, n_test=30, n_wts=10,
                        n_clusters=10, n_components_filter=15, bad_prop=0.1, knn=5)