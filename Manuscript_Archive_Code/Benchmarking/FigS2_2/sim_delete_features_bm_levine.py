...
def run_delete_features(df1, df2, x_labels, y_labels, features_to_delete,
    ...
    for i, feature in enumerate(features_to_delete):
        ...
        _ = match.match_cells('ovlp', sparsity=5000)
        ...
        _ = match.match_cells('all', sparsity=5000)
        res['all_acc'].append(eval_matching_accuracy(x_labels, y_labels, match.matching['all'], 'maj'))
    
        # dont need to run matchability test as we now this can be integrated
...
if __name__ == "__main__":
    ...
    path = 'data/feature_deletion/bm_levine/sxc/'
    ...
    n1, _ = df1.shape
    n2, _ = df2.shape
    df1 = df1.iloc[np.random.choice(n1, 8000, replace=False), :] # total 40k cells
