...
sam = function(orig_fname='data/feature_deletion/orig', 
  ...
  for (i in 0:n_idx) {
    # add by bkz
    ####
    orig_data = read_csv(paste0(orig_fname, '_', data_idx, i, '.csv'), col_types = cols())
...
slt = function(orig_fname='data/feature_deletion/orig', 
  ...
  for (i in 0:n_idx) {
    ...
    x_label_vec = read_csv(paste0(orig_fname, '_x', i, '.csv'), col_types = cols()) %>% select(label) %>% pull()
    y_label_vec = read_csv(paste0(orig_fname, '_y', i, '.csv'), col_types = cols()) %>% select(label) %>% pull()
...
### note kbet was not used in this benchmarking so was not adjusted for liger 
...
mix = function(orig_fname='data/feature_deletion/orig', 
  ...
  for (i in 0:n_idx) {
    ...
    ## added for liger by bkz
    ##########
    x_labels = read_csv(paste0(orig_fname, '_x', i, '.csv'), col_types = cols()) %>% select(label) %>% pull()
    y_labels = read_csv(paste0(orig_fname, '_y', i, '.csv'), col_types = cols()) %>% select(label) %>% pull()
